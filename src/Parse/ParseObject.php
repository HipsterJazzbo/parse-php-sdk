...
class ParseObject implements Encodable
{
    ...
    public static function create($className, $objectId = null,
                                  $isPointer = false)
    {
        if (static::hasRegisteredSubclass($className)) {
    ...
    public static function registerSubclass()
    {
        if (isset(static::$parseClassName)) {
            if (! static::hasRegisteredSubclass(static::$parseClassName)) {
                $customSubclass = get_called_class();
                static::validateSubclass($customSubclass, '_User', 'Parse\ParseUser');
                static::validateSubclass($customSubclass, '_Role', 'Parse\ParseRole');

                self::$registeredSubclasses[static::$parseClassName] = $customSubclass;
            }
        } else {
            throw new \Exception(
    ...
    private static function validateSubclass($customSubclass, $parseClassName, $parseSubclass)
    {
        if (static::$parseClassName == $parseClassName &&
            ! (is_subclass_of($customSubclass, $parseSubclass) || $customSubclass == $parseSubclass)
        ) {
            throw new \Exception("Subclasses of $parseClassName must extend $parseSubclass");
        }
    }
